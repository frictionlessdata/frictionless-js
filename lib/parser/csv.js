...
const csvParser = async function csvParser(file) {
  ...
  if (!(typeof window === 'undefined')) {
    ...
    const s = new Readable(); // Create a transform stream with our transformer
    ...
    const lineStream = stream.pipeThrough(ts); // Read the stream of strings
...
const guessParseOptions = async file => {
  ...
  let text = ''; // We assume that reading first 1M bytes is enough to detect delimiter, line terminator etc.:
  ...
  if (file.displayName === 'FileLocal') {
    ...
    const stream = await file.stream();
    let bytes = 0; // Check if it is running on nodejs or browser:
