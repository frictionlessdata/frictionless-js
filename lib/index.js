...
class Dataset {
  ...
  static async load(pathOrDescriptor, {owner = null} = {}) {
    ...
    await dataset._sync()
    dataset._descriptor.encoding = dataset.encoding || DEFAULT_ENCODING
    return dataset
  }
  ...
  get encoding() {
    console.log(this.descriptor.title)
    return chardet.detect(new Buffer(JSON.stringify(this.descriptor)));
  }
